...
const CommentModal = ({
  ...
  return (
    <SideModal showModal={showCommentModal} setShowModal={setShowCommentModal}>
      <div className="relative flex flex-col w-full sm:w-[28rem] h-full bg-secondary-2 border rounded-lg shadow-md">
        <div className="flex justify-between items-center px-4 h-16 text-white border-b rounded-t-lg">
          {comments && comment ? (
            <button
              onClick={() =>
                router.replace(
        ...
        {comments && position && (
          <div className="flex flex-1 flex-col space-y-5 px-3 py-6 overflow-scroll bg-secondary-2 text-white mb-2">
            {comments.filter(
        ...
        {comments && comment && (
          <div className="flex flex-1 flex-col space-y-5 bg-secondary-2 px-3 py-6 overflow-scroll">
            {comment && (
              <Comment
                key={comment}
        ...
        {!comment && (
          <form
            ...
            className="h-60 w-full p-4 bg-secondary-1 rounded-b-lg"
          >
            <textarea
              name="comment"
              className="w-full h-full resize-none text-sm bg-secondary-1 text-white placeholder:text-primaryfocus:outline-none border-none focus:ring-0"
            ...
            />
            <button className="absolute bottom-4 right-4 w-10 h-10 flex items-center justify-center border border-white bg-primary rounded-full hover:scale-105 active:scale-100 transition-all">
              {submitting ? (
                <LoadingDots color="#FFF" />
              ) : (
                <Send className="w-4 h-4 mt-0.5 mr-0.5 text-white" />
              )}
            </button>
          </form>
        )}
